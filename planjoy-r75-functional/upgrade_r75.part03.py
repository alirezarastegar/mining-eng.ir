...
function bindGlobal(){
  ...
  document.addEventListener('click',async e=>{
    ...
    if(sectionBtn){settingsSection=sectionBtn.dataset.settingsSection;shell();return;}
...
if 'planner tab click failed' not in main:
    ...
    exec_end=main.index('\n',exec_pos)+1
    nav_cert="""  // Real click navigation must survive repeated complete DOM re-renders.\n  const navigation=await exec(`let stage='start';try{stage='routes';const routes=['today','calendar','goals','focus','review','stats','countdown','widgets','joy','smart','settings'];for(const r of routes){stage='route:'+r;const b=document.querySelector('[data-route=\\\"'+r+'\\\"]');if(!b)throw new Error('route missing '+r);b.click();await new Promise(x=>setTimeout(x,100));const active=document.querySelector('[data-route=\\\"'+r+'\\\"].active');const page=document.querySelector('.page-'+r);if(!active||!page||document.body.dataset.route!==r)throw new Error('route click failed '+r)}stage='planner-open';document.querySelector('[data-route=\\\"today\\\"]').click();await new Promise(x=>setTimeout(x,100));for(const v of ['list','today','quadrant','timeline']){stage='planner:'+v;const b=document.querySelector('[data-view=\\\"'+v+'\\\"]');if(!b)throw new Error('planner tab missing '+v);b.click();await new Promise(x=>setTimeout(x,110));if(!document.querySelector('[data-view=\\\"'+v+'\\\"].active'))throw new Error('planner tab click failed '+v)}stage='settings-open';document.querySelector('[data-route=\\\"settings\\\"]').click();await new Promise(x=>setTimeout(x,110));for(const s of ['general','appearance','language','notifications','data','sync','privacy','updates']){stage='settings:'+s;const b=document.querySelector('[data-settings-section=\\\"'+s+'\\\"]');if(!b)throw new Error('settings tab missing '+s);b.click();await new Promise(x=>setTimeout(x,90));if(!document.querySelector('.settings-section[data-section=\\\"'+s+'\\\"].active'))throw new Error('settings tab click failed '+s)}stage='restore-list';document.querySelector('[data-route=\\\"today\\\"]').click();await new Promise(x=>setTimeout(x,100));document.querySelector('[data-view=\\\"list\\\"]')?.click();await new Promise(x=>setTimeout(x,100));return {ok:true,stage:'done'}}catch(e){return {ok:false,stage,error:String(e?.message||e),stack:String(e?.stack||'')}}`);if(!navigation?.ok)throw new Error('navigation certification failed '+JSON.stringify(navigation));\n"""
...
new_recur="await exec(`document.querySelector('[data-route=\\\"today\\\"]').click();await new Promise(r=>setTimeout(r,110));document.querySelector('[data-view=\\\"list\\\"]')?.click();await new Promise(r=>setTimeout(r,110));const row=[...document.querySelectorAll('[data-task-row]')].find(x=>x.dataset.taskRow==='${tid}');if(!row)throw new Error('cert recurrence row missing');row.querySelector('[data-action=\\\"toggle-task\\\"]').click();await new Promise(r=>setTimeout(r,240));return true`);"
